...
def getroom(request):
    ...
    return JsonResponse({'n_to': room.n_to, 's_to': room.s_to, 'e_to': room.e_to, 'w_to': room.w_to, 'x': room.x, 'y': room.y}, safe=True)
...
def move(request):
    ...
    if nextRoomID is not None and nextRoomID > 0:
        ...
        return JsonResponse({'name': player.user.username, 'title': nextRoom.title, 'description': nextRoom.description, 'players': players, 'error_msg': "", 'room_id': nextRoom.id}, safe=True)
    else:
        players = room.playerNames(player_id)
